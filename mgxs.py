...
#where neutron start to propagate in geometry ?
...
groups = mgxs.EnergyGroups(group_edges = XMAS172)

#create variable that contain all cells
...
# Add all cell/reaction combination to tally list
for cell in openmc_cells:
...
#Load all cell/reaction combination 
for cell in openmc_cells:
    for rxn_type in xs_library[cell.id]:
        xs_library[cell.id][rxn_type].load_from_statepoint(sp)
 
#xs_library contain all cell/reaction combination       
nufission = xs_library[fuel.id]['nu-fission']
nufission.print_xs(xs_type='macro', nuclides=['U235', 'U238'])

nufission = xs_library[moderator.id]['nu-scatter']
nufission.print_xs(xs_type='macro', nuclides=['O16'])
